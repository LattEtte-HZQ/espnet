...
nj=56                   # The number of parallel jobs.
...
token_type=phn      # Tokenization type (char or bpe).
...
# ASR model related
asr_task=asr_transducer_calphn   # ASR task mode. Either 'asr' or 'asr_transducer' or 'asr_transducer_mtp'.
...
pred_num=2  # Number of predicted tokens
...
batch_size=1
inference_tag=hzqtest    # Suffix to the result dir for decoding.
inference_config= # Config for decoding.
inference_args=   # Arguments for decoding, e.g., "--lm_weight 0.1".
                  # Note that it will overwrite args in inference config.
inference_lm=valid.loss.ave.pth       # Language model path for decoding.
inference_ngram=${ngram_num}gram.bin
inference_asr_model=valid.cer_transducer.best.pth # ASR model path for decoding.
                                      # e.g.
...
g2p="pypinyin_g2p_phone"    # g2p method (needed if token_type=phn).
...
Options:
    ...
    --skip_stages        # Spicify the stagefs to be skipped (default="${skip_stages}").
    ...
    # ASR model related
    --asr_task         # ASR task mode. Either 'asr' or 'asr_transducer' or 'asr_transducer_mtp'. (default="${asr_task}").
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ] && ! [[ " ${skip_stages} " =~ [[:space:]]5[[:space:]] ]]; then
    ...
    # HZQ JOB
        ...
        # The first symbol in token_list must be "<blank>" and the last must be also sos/eos:
        # 0 is reserved for CTC-blank for ASR and also used as ignore-index in the other task
        ${python} -m espnet2.bin.tokenize_phoneme  \
            --token_type "${token_type}" \
            --input "${data_feats}/lm_train.txt" --output "${token_list}" ${_opts} \
            --field 2- \
            --cleaner "${cleaner}" \
