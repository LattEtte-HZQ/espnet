...
def inference(
    ...
    ave_phn_duration = dict()
    with open("./data/zh_token_list/phn/tokens.txt", 'r', encoding='utf-8') as file:
        for line in file:
            key = line.strip()
            if key:
                ave_phn_duration[key] = [0, 0.0]
    # 4 .Start for-loop
    with (DatadirWriter(output_dir) as writer):
        sample_cnt = 0
        for keys, batch in loader:
            sample_cnt += 1
            logging.info(f"Sample_cnt: {sample_cnt}")
            ...
            try:
                    ...
                    for i in range(0, decoding_steps + 1, 1):
                        ...
                        if i == decoding_steps:
                            ...
                            if display_hypotheses:
                                ...
                                #     logging.info(
                                #         f"Token{j} current best hypothesis (0-{_length}ms): "
                                #         f"{keys}: {_result[j][0][0]}"
                                #    )
                                temp_hyp = _result[0][0][3]
                                now_length = len(temp_hyp.yseq)
                                now_step = i + 1
                                if now_length > pre_length:
                                    now_phn = _result[0][0][1][-1]
                                    if now_phn in ave_phn_duration.keys():
                                        ave_phn_duration[now_phn][0] += 1
                                        ave_phn_duration[now_phn][1] += (now_step - pre_step) * decoding_window
                                    all_duration += (now_step - pre_step) * decoding_window
                                    for j in range(pred_num):
                                        durations[j].append((now_step - pre_step) * decoding_window)
                                    pre_length = now_length
                                    pre_step = now_step
                    logging.info
                    (
                        f"Average phn duration:{all_duration * 1.0 / decoding_steps}ms"
                    )
                else:
                ...
                # for j in range(pred_num):
                #     logging.info(f"Token_{j + 1} Phoneme: {results[j][0][1]}")
                #     logging.info(f"Token_{j + 1} Duration: {durations[j]}ms")
                # if display_hypotheses:
                #     for j in range(pred_num):
                #         logging.info(
                #             f"Token_{j + 1} Final best hypothesis: {keys}: {results[j][0][0]} Time cost {time.time() - start_time} seconds"
                #         )
            ...
            # key = keys[0]
            # for j in range(pred_num):
            #     ibest_writer = writer[f"Token_{j}_best_recog"]
            #     for text, token, token_int, hyp in results[j]:
            ...
            if sample_cnt > 1 and (sample_cnt%100) == 0:
                logging.info(f"For past {sample_cnt} samples")
                for k, v in ave_phn_duration.items():
                    if v[1] != 0.0:
                        logging.info(f"Phoneme {k} duration {v[1] / v[0]}")
                    else:
                        logging.info(f"Phoneme {k} not found in past {sample_cnt} samples")
